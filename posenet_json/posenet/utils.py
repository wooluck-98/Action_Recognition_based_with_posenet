...
def draw_skel_and_kp(
    ...
    adjacent_keypoints = []
    cv_keypoints = []
    col = [(255,0,0), (0,255,0), (0,0,255), (255,255,0), (255,0,255), (0,255,255), (139,0,139)]
    for ii, score in enumerate(instance_scores):
        ...
        for ks, kc in zip(keypoint_scores[ii, :], keypoint_coords[ii, :, :]):
            if ks < min_part_score:
                continue
            cv_keypoints.append(cv2.KeyPoint(kc[1], kc[0], 10. * ks))
    # out_img = np.zeros(out_img.shape, np.uint8)
    out_img = cv2.drawKeypoints(
        out_img, cv_keypoints, outImage=np.array([]), color=(255, 255, 0),
        flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

    # print(adjacent_keypoints)
    for idx, keypoint in enumerate(adjacent_keypoints): 
        if len(keypoint):
            out_img = cv2.line(out_img, tuple(keypoint[0]), tuple(keypoint[1]), color=col[idx%7], thickness=3)
    return out_img
